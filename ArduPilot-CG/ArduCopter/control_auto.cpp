...
void Copter::auto_takeoff_run()
{
    ...
    //## Set the pitch angle to zero and use the 5th DOF for forward control instead
    attitude_control.input_euler_angle_roll_pitch_euler_rate_yaw(wp_nav.get_roll(), 0, target_yaw_rate, get_smoothing_gain());

    //## Use the pitch angle as input for the forward thrust.
    attitude_control.input_thrust_forward(wp_nav.get_pitch());
}
...
//## auto_wp_run_heading - runs the auto waypoint controller with heading control
//      called by auto_run at 100hz or more
void Copter::auto_wp_run_heading()
{
    // if not auto armed or motor interlock not enabled set throttle to zero and exit immediately
    if (!motors.armed() || !ap.auto_armed || !motors.get_interlock()) {
        // clear i term when we're taking off
    ...
    // process pilot's forward (pitch) input
    float forward_throttle = 0;
    if (!failsafe.radio) {
        // get pilot's desired yaw rate
        forward_throttle = channel_pitch->get_control_in();
    ...
    // run waypoint controller
    float yaw = wp_nav.update_wpnav_heading();

    // call z-axis position controller (wpnav should have already updated it's alt target)
    pos_control.update_z_controller();

    // call attitude and yaw controller
    //## Attitude is always kept in level. Navigation is done by changing heading.
    attitude_control.input_euler_angle_roll_pitch_yaw(0, 0, yaw, true, get_smoothing_gain());

    // set forward thurst
    attitude_control.input_thrust_forward(forward_throttle);
}
