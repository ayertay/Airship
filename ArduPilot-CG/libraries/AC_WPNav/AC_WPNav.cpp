...
// their values.
//
AC_WPNav::AC_WPNav(const AP_InertialNav& inav, AP_AHRS& ahrs, AC_PosControl& pos_control, const AC_AttitudeControl& attitude_control) :
    _inav(inav),
    _ahrs(ahrs),
...
/// update_wpnav_heading - run the wp heading controller - should be called at 100hz or higher
//## A modified updated wpnav function that uses heading instead of quad copter pos_control.
//## Inspired by AP_L1_Control::update_waypoint
float AC_WPNav::update_wpnav_heading()
{
    // calculate dt
    float dt = _wp_last_update - AP_HAL::millis();

    // Get current position and velocity
    struct Location current_loc;
    _ahrs.get_position(current_loc);
    _groundspeed = _ahrs.groundspeed();
    
 
    // Get the direction between the previous and next waypoint as and make it a unit vector. 
    Vector2f wp_dir;
    wp_dir.x = _destination.x -_origin.x;
    wp_dir.y = _destination.y -_origin.y;

    // Check for wp_dir zero length and track directly to the destination
    // if too small
    if (wp_dir.length() < 1.0e-6f) {
        wp_dir.x = _destination.x - current_loc.lat;
        wp_dir.y = _destination.y - current_loc.lng;
        if (wp_dir.length() < 1.0e-6f) {
            wp_dir = Vector2f(cosf(_ahrs.yaw), sinf(_ahrs.yaw));
        }
    }
    ...
    // Calculate the the position relative to the previous wp
    Vector2f rel_pos;
    rel_pos.x = current_loc.lat - _origin.x;
    rel_pos.y = current_loc.lng - _origin.y;

    // Calculate distance to target track by taking the cross product between the direction wp and relative position
    _crosstrack_error = rel_pos % wp_dir;


    //## P part of the controller, actually speed dependent
    float sine_P_angle = _P_gain * _crosstrack_error * _groundspeed;
    //Limit sine of Nu1 to provide a controlled track capture angle of 45 deg
    sine_P_angle = constrain_float(sine_P_angle, -0.7071f, 0.7071f);
    float P_angle = asinf(sine_P_angle);
    ...
    if (_i_gain <= 0 || !is_equal(_i_gain, _i_gain_prev)) {
        _i_angle = 0;
        _i_gain_prev = _i_gain;
    } else if (fabsf(P_angle) < radians(5)) {
        _i_angle += _P_gain * _i_gain * dt;

        // an AHRS_TRIM_X=0.1 will drift to about 0.08 so 0.1 is a good worst-case to clip at
        _i_angle = constrain_float(_i_angle, -0.1f, 0.1f);
    }
    
    // Sum up angles, follow and correction
    _nav_bearing = _ahrs.yaw + _P_gain + _i_angle; // Wanted bearing (radians)
