...
const AP_Param::GroupInfo AP_MotorsSingle::var_info[] = {
    ...
    // @Values: 50, 125, 250
    AP_GROUPINFO("SV_SPEED", 43, AP_MotorsSingle, _servo_speed, AP_MOTORS_SINGLE_SPEED_ANALOG_SERVOS),
...
void AP_MotorsSingle::output_armed_stabilizing()
{
    ...
    float rp_thrust_max = fabsf(roll_thrust)+ fabsf(pitch_thrust);
 ...
 rp_scale = constrain_float( fabsf( 0.1f * throttle_thrust) / ( fabsf(roll_thrust)+ fabsf(pitch_thrust) + fabsf(yaw_thrust) ), 0.0f , 1.0f);
        if (rp_scale < 1.0f) {
    ...
    actuator[0] = rp_scale * (-roll_thrust - pitch_thrust - yaw_thrust) - 0.1f* throttle_thrust;
    // right servo
    actuator[1] = rp_scale * (roll_thrust - pitch_thrust + yaw_thrust) + 0.1f* throttle_thrust;
    // rear servo
    actuator[2] = rp_scale * (roll_thrust + pitch_thrust - yaw_thrust) - 0.1f* throttle_thrust;
    // left servo
    actuator[3] = rp_scale * (-roll_thrust + pitch_thrust + yaw_thrust) + 0.1f* throttle_thrust;
